...
export interface HeroCarouselItem {
  ...
  id?: string | number;
  /** Headline for the active slide. `\n` becomes a separate reveal line. */
  title: string;
  /** Image URL, used both in the card and as the graded background. */
  image: string;
  /** Byline printed beside the headline, e.g. "BY AURELIA STUDIO." */
  credit?: string;
  /** Right-aligned facts, e.g. ["SAT NOV 15", "5-10 PM", "MIAMI"]. */
  meta?: string[];
  /** CSS colour the background is graded to. */
  accent?: string;
  /** Optional link — clicking an already-focused card navigates here. */
  href?: string;
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {brand ? (
          <div className="font-semibold tracking-[0.06em]" style={{ fontSize: label * 1.35 }}>
            {brand}
          </div>
        ) : null}
        {onMenu ? (
        ...
        <div className="flex w-full flex-wrap items-end gap-x-[6vw] gap-y-2">
          ...
          {active.href ? (
            <motion.a
              key={`link-${index}`}
              href={active.href}
              className="inline-flex items-center gap-1 font-mono text-[0.85em] uppercase tracking-[0.14em] opacity-80 transition-opacity hover:opacity-100"
              style={{ fontSize: label * 1.3 }}
              initial={{ opacity: 0 }}
              animate={{ opacity: 0.8 }}
